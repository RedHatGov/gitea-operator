...
function update_components() {
    ...
    if [ -z "$components_updated" ]; then
        if [ "$VIRTUAL_ENV" ]; then
            error_run "Updating the Operator SDK manager" 'pip install --upgrade -r "$SCRIPT_ROOT/requirements.txt"' || return 1
        else
            error_run "Updating the Operator SDK manager" 'pip install --user --upgrade -r "$SCRIPT_ROOT/requirements.txt"' || return 1
        fi
